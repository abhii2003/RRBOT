...
client = vonage.Client(key="568f97cb", secret="0jo5DLN9THh7zJaR")
sms = vonage.Sms(client)
last_detection_time = None
# Function to generate random GPS location
...
# Function to upload GPS location to Superbase
def upload_to_superbase(latitude, longitude):

    API_URL = 'https://mhjzrgamfdvvtczgxokb.supabase.co'
    API_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1oanpyZ2FtZmR2dnRjemd4b2tiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MTQzNTk1OTksImV4cCI6MjAyOTkzNTU5OX0.hjjxh1aA8o7221iyAKRxzYZiEpFhNyOn07QrvWIcg3U'
    ...
    supabase.table('location').insert(data).execute()  # inserting one record
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        if (s[13:19] == "person"):
            # Check if at least 30 seconds have passed since the last detection
            if last_detection_time is None or time.time() - last_detection_time >= 40:
                # Send SMS with random GPS location when a person is detected
                lat, lon = generate_random_location()
                f = 1
                upload_to_superbase(lon, lat)
                if (f == 1):
                    responseData = sms.send_message(
                        {
                            "from": "Vonage APIs",
                            "to": "919360397804",
                            "text": f"A person was detected at longitude : {lon} , latitude : {lat} ",
                        }
                    )
                    f = 0
                ...
                # Update the last detection time to the current time
